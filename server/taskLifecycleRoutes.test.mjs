...
test('project task preview can enforce the deployment public-project policy', async () => {
  const f = taskFixture()
  f.deps.canView = (project, userId) => project.userId === userId
    || project.admins.includes(userId) || project.team.includes(userId)
  await assert.rejects(
    getProjectTaskPreview({ taskId: 't1', userId: 'public-only-viewer' }, f.deps),
    failsWith('not-authorized'),
  )
  const member = await getProjectTaskPreview({ taskId: 't1', userId: 'member' }, f.deps)
  assert.equal(member.payload._id, 't1')
})
