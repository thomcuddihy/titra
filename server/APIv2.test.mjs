...
test('v2 capabilities evolve to the v7 security contract without changing the API envelope', () => {
  const value = contracts.apiV2Capabilities()
  assert.equal(value.apiVersion, 2)
  assert.equal(value.capabilitiesVersion, 3)
  ...
  assert.equal(value.features.security.policyDiscovery, 1)
  ...
  assert.equal(value.contracts.security.version, 1)
  assert.equal(value.contracts.security.releaseProfile, 'security-v7')
  assert.equal(value.contracts.security.apiTokens.storage, 'sha256-domain-separated')
  assert.deepEqual(value.contracts.security.authentication.verificationDeadlineEnforced, [
    'httpApi', 'ddpMethods', 'ddpPublications',
  ])
  assert.equal(value.contracts.security.integrations.browserCredentialPublication, 'disabled')
  assert.equal(value.contracts.security.publicProjects.operatorDisableEnforcedServerSide, true)
  ...
  assert.deepEqual(value.deployment.security, {
    hstsEnabled: false,
  ...
  assert.equal(
    contracts.apiV2Capabilities({
      ...
      hstsEnabled: true,
      oidcVerifiedEmailLinkingEnabled: true,
      oauthEncryptionConfigured: true,
      publicProjectsDisabled: true,
      unsafeLegacyScriptsEnabled: true,
    }).deployment.webhookActionVerificationEnabled,
    true,
  )
  assert.deepEqual(contracts.apiV2Capabilities({
    hstsEnabled: true,
    oidcVerifiedEmailLinkingEnabled: true,
    oauthEncryptionConfigured: true,
    publicProjectsDisabled: true,
