...
async function getProjectTaskPreview({ taskId, userId }, { findTask, findProject, inspectReferences }) {
  const task = await findTask({ _id: taskId, projectId: { $exists: true } })
  const project = task && await findProject({ _id: task.projectId })
  if (!task || !canViewProject(project, userId)) throw taskError('not-authorized', 'Task not found.')
  return {
