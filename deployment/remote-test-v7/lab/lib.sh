...
LOADED_CANDIDATE_STATE='__V7_STATE_ROOT__/production-images/loaded-v7-__V7_PACKAGE_RELEASE_ID__.env'
LOADED_MONGO_STATE='__V7_STATE_ROOT__/production-images/loaded-mongo-v7-__V7_PACKAGE_RELEASE_ID__.env'
...
load_release_manifest() {
  ...
  while IFS= read -r raw || [[ -n "$raw" ]]; do
    ...
    case "$key" in
      RELEASE_FORMAT|PACKAGE_RELEASE_ID|RELEASE_PROFILE|TITRA_VERSION|TITRA_TEST_IMAGE|TITRA_TEST_IMAGE_ID|TITRA_CONFIG_IMAGE_ID|TITRA_IMAGE_ARCHIVE|MONGO_TEST_IMAGE|MONGO_TEST_IMAGE_ID|MONGO_CONFIG_IMAGE_ID|MONGO_SOURCE_DIGEST|MONGO_ATTESTATION_MANIFEST_ID|MONGO_IMAGE_ARCHIVE|SOURCE_COMMIT|SOURCE_CONTEXT_SHA256|TITRA_IMAGE_EVIDENCE_DIRECTORY|TITRA_IMAGE_EVIDENCE_SHA256SUMS_SHA256|STOCK_IMAGE|STOCK_IMAGE_ID|V5_IMAGE|V5_IMAGE_ID|V6_IMAGE|V6_IMAGE_ID|V6_CONFIG_IMAGE_ID|V6_IMAGE_ARCHIVE|PREVIOUS_V7_IMAGE|PREVIOUS_V7_IMAGE_ID|PREVIOUS_V7_CONFIG_IMAGE_ID)
        ;;
      *)
  ...
  [[ "${RELEASE_VALUES[MONGO_ATTESTATION_MANIFEST_ID]}" == none ||
    "${RELEASE_VALUES[MONGO_ATTESTATION_MANIFEST_ID]}" =~ ^sha256:[0-9a-f]{64}$ ]] ||
    die 'The lab package Mongo attestation identity is invalid.'
  if [[ "${RELEASE_VALUES[PREVIOUS_V7_IMAGE]}" != none ||
    "${RELEASE_VALUES[PREVIOUS_V7_IMAGE_ID]}" != none ||
    "${RELEASE_VALUES[PREVIOUS_V7_CONFIG_IMAGE_ID]}" != none ]]; then
    [[ "${RELEASE_VALUES[PREVIOUS_V7_IMAGE]}" =~ ^[A-Za-z0-9][A-Za-z0-9._/:@-]{0,254}$ &&
      "${RELEASE_VALUES[PREVIOUS_V7_IMAGE]}" != none &&
      "${RELEASE_VALUES[PREVIOUS_V7_IMAGE_ID]}" =~ ^sha256:[0-9a-f]{64}$ &&
      "${RELEASE_VALUES[PREVIOUS_V7_CONFIG_IMAGE_ID]}" =~ ^sha256:[0-9a-f]{64}$ ]] ||
      die 'The lab previous-v7 admission must be complete and valid or all none.'
    for key in STOCK_IMAGE_ID V5_IMAGE_ID V6_IMAGE_ID V6_CONFIG_IMAGE_ID TITRA_TEST_IMAGE_ID TITRA_CONFIG_IMAGE_ID; do
      [[ "${RELEASE_VALUES[$key]}" != "${RELEASE_VALUES[PREVIOUS_V7_IMAGE_ID]}" &&
        "${RELEASE_VALUES[$key]}" != "${RELEASE_VALUES[PREVIOUS_V7_CONFIG_IMAGE_ID]}" ]] ||
        die 'The lab previous-v7 admission overlaps another generation or candidate.'
    done
  fi
